...
def bind(src, dest, chroot, create=False, log=None, readonly=False, recursive=False, optional=False, **_kwargs):
    ...
    if create:
        ...
        if not os.path.isdir(src) and src not in fstypes:
            ...
            try:
                ...
                raise ChrootMountError("cannot bind mount to '{}'".format(dest), getattr(e, "errno", None))
    ...
    try:
        mount(source=src, target=dest, fstype=fstype, flags=reduce(operator.or_, mount_flags, 0), data=",".join(mount_options))
        if readonly:
            mount_flags.extend([MS_REMOUNT, MS_RDONLY])
            mount(source=src, target=dest, fstype=fstype, flags=reduce(operator.or_, mount_flags, 0), data=",".join(mount_options))
    except OSError as e:
        raise ChrootMountError("Failed mounting: mount -t {} {} {}".format(fstype, src, dest), e.errno)
...
class Chroot(SplitExec):
    ...
    def __init__(
        self, path: str, log: None = None, mountpoints: Optional[Dict[str, Dict[str, bool]]] = None, hostname: Optional[str] = None, skip_chdir: bool = False
    ) -> None:
        ...
        if not os.path.isdir(self.path):
            raise ChrootError("cannot change root directory to '{}'".format(path), errno.ENOTDIR)
            ...
            if source.startswith("$"):
                ...
                if src == source:
                    if "optional" in opts:
                        ...
                        raise ChrootMountError("cannot mount undefined environment variable: {}".format(source))
    ...
    def _cleanup(self) -> None:
        for _, _, chrmount, opts in self.mounts:
            ...
            try:
                ...
                raise ChrootMountError("failed to remove chroot mount point '{}'".format(chrmount), getattr(e, "errno", None))
